...
| s > 0, R small, A small | always fails    | always fails                   | no large o. component on the right |
| s = 0, R mixed, A small | always fails    | always fails                   | no large o. component on the left  |
| s > 0, R mixed, A small | 8×s×B = 8×R     | 8×s×B = 8×R ∧ L×R = -L×k×A     | [1]                                |
| s = 0, R small, A mixed | always fails    | always fails                   | no large o. component on the left  |
| s > 0, R small, A mixed | 8×s×B = 8×k×A   | 8×s×B = 8×k×A ∧ L×R = -L×k×A   | symmetric of [1]                   |
| s = 0, R mixed, A mixed | 8×R = -8×k×A    | R = - k×A                      | hard to test (hash inversion)      |
